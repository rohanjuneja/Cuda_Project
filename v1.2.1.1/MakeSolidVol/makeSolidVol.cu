...
__global__ void first_kernel(unsigned char *d_vol)
{
	long idx=blockDim.x*blockIdx.x+threadIdx.x;
	if(d_vol[idx] != 0) {
			d_vol[idx] = OBJECT_VAL;
		}
}
__global__ void second_kernel(unsigned char *vol, unsigned char backgroundVal, unsigned char objectVal)
{
	long idx=blockDim.x*blockIdx.x+threadIdx.x;
	...
	unsigned char backgroundVal, unsigned char objectVal) 
{
	...
	int i;
	unsigned char* d_vol;
	cudaMalloc((void **)&d_vol,sizeof(unsigned char)*L*M*N);
	cudaMemcpy(d_vol,vol,sizeof(unsigned char)*L*M*N,cudaMemcpyHostToDevice);
	first_kernel<<<M*N,L>>>(d_vol);
	cudaMemcpy(vol,d_vol,sizeof(unsigned char)*L*M*N,cudaMemcpyDeviceToHost);
	...
	// 	}
	// }
	kernel_for_z_planes<<<N,1>>>(d_vol,L,M,N);
	...
	kernel_for_y_planes<<<M,1>>>(d_vol,L,M,N);
	...
	kernel_for_x_planes<<<L,1>>>(d_vol,L,M,N);
	...
	// cudaMemcpy(d_vol,vol,sizeof(unsigned char)*L*M*N,cudaMemcpyHostToDevice);
	second_kernel<<<M*N,L>>>(d_vol, backgroundVal, objectVal);
	cudaMemcpy(vol,d_vol,sizeof(unsigned char)*L*M*N,cudaMemcpyDeviceToHost);
