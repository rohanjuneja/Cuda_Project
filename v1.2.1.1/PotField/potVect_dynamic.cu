...
struct reduceStruct {
	 __device__ Vector operator()(Vector a, Vector b) {
		Vector temp;
		temp.xd=a.xd+b.xd;
		temp.yd=a.yd+b.yd;
		temp.zd=a.zd+b.zd;
...
*/
__global__ void dynamic_kernel(int i,int j,int k,int fieldStrenght,Vector* result,int size)
{
	...
	if(s<size)
	{
			...
			result[s].zd=(v3/r);
			if(s==6554)
			printf("child kernel:%d %d %.6f %.6f %.6f %.6f %.6f %.6f\n",s,size,d_bound[4].x,d_bound[4].y,d_bound[4].z,v1,v2,v3);
...
__global__ void compute_potential_field(Vector* force,int numBound,unsigned char* f,bool inOut,int slsz,int sz,int L, int fieldStrenght)
{
	...
	//result=(Vector *)malloc(sizeof(Vector)*size);
	Vector *result;
	cudaMalloc((void **)&result,sizeof(Vector)*size);
	
	// cudaStream_t stream;
	// cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking);
	// dynamic_kernel<<<dimGrid,dimBlock>>>(i,j,k,fieldStrenght,result,size);

	//for(int i=0; i<size; i++) {
	//	printf("%.6f %.6f %.6f\n", result[i].xd, result[i].yd, result[i].zd);
	//}

	// cudaError_t err;
	// cudaStreamSynchronize(s);
	// cudaStreamDestroy(stream);
	// printf("Rohan before cuda device sync\n");
	...
	// }
	// printf("Rohan after cuda device sync\n");
	// printf("Rohan:%d %.6f %.6f %.6f\n",size,result[0].xd,result[0].yd,result[0].zd);
		for(int j=0;j<size;j++)
			printf("%.6f %.6f %.6f\n",result[j].xd,result[j].yd,result[j].zd);
	// thrust::device_ptr<Vector> cptr = thrust::device_pointer_cast(result);
	//  // thrust::device_ptr<cufftReal> dev_ptr(out1_d); 
 //        thrust::device_vector<Vector> val_vec(size); 
 //        thrust::copy(thrust::seq,cptr, cptr + size, val_vec.begin()); 
	// // thrust::device_vector<Vector> cptr(result,result+size);
	reduceStruct func;
	Vector init;
	init.xd=1;
	init.yd=1;
	...
	Vector final;
	// Vector final= thrust::reduce(thrust::seq,result, result+size,init,func);
	// if(final.xd!=0)
		// printf("%.6f %.6f %.6f\n",final.xd,final.yd,final.zd);
	
	final = init;
	if(final.xd!=0)
		printf("%.6f %.6f %.6f\n",init.xd,init.yd,init.zd);
	force[idx].xd=init.xd;
	force[idx].yd=init.yd;
	force[idx].zd=init.zd;
